...
from services.rag.text_processor import SUPPORTED_EXTENSIONS, TextCleanerTransformer
...
class DocumentLoader:
    ...
    @staticmethod
    def load_directory_records(
        dir_path: str | Path,
        *,
        apply_cleaner: bool = True,
    ) -> List[Dict[str, Any]]:
        docs = DocumentLoader.load_directory(dir_path, apply_cleaner=apply_cleaner)
        return [DocumentLoader.to_record(doc) for doc in docs]

    @staticmethod
    def to_record(doc: Document) -> Dict[str, Any]:
        return {"text": doc.page_content, "metadata": dict(doc.metadata)}
